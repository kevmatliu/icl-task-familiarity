...
def identity_cov(d, strength, device=config.DEVICE, dtype=torch.float32):
    return torch.eye(d, device=device, dtype=dtype)


def exp_cov(d, strength, device=config.DEVICE, dtype=torch.float32):
    # task diversity K
...
def powerlaw_cov(d, strength, device=config.DEVICE, dtype=torch.float32):
    idx = torch.arange(d, device=device, dtype=dtype)
...
def sample_task_weights(W, d, k, dtype=torch.float32):
    e_val, e_vec = torch.linalg.eigh(W)
